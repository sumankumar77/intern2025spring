...
def _props_grouper(props, pgs):
    ...
    props_by_groups = map(lambda item: sorted(
        item, key=lambda item_: item_[0][0]), props_by_groups)
    props = []
...
def restore_quoted_url(css, css_orig):
    urls_orig = re.findall('background-image:\s*url\((.*?)\)', css_orig)
    urls = re.findall('background-image:url\((.*?)\)', css)
    if len(urls_orig) == len(urls):
        for url, url_orig in zip(urls, urls_orig):
            css = css.replace(url, url_orig)
            # re.findall(url, css)
            # print(url.decode('utf-8'))
    else:
        print('Number of ULRs not matched!')
    return css
...
def css_minify(css, wrap=False, comments=False, sort=False, noprefix=False):
    ...
    css = restore_quoted_url(css, css_copy)
...
def condense_html_whitespace(html):
    ...
    tagsStack = []
    split = re.split('(<\\s*pre.*>|<\\s*/\\s*pre\\s*>|<\\s*textarea.*>|<\\s*/\\s*textarea\\s*>)',
                     html, flags=re.IGNORECASE)
    for i in range(0, len(split)):
        if (i + 1) % 2 == 0:    # if we are on a tag
            tag = rawtag(split[i])
            if tag.startswith('/'):
            ...
            continue
        if not tagsStack:    # else check if we are outside any nested <pre>/<textarea> tag
...
def process_single_js_file(js_file_path, timestamp=False, overwrite=False,
                           zipy=False, output_path=None, add_hash=False):
